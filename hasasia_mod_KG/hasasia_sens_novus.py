...
if __name__ == '__main__':
    data_path = os.path.expanduser('~/novus/.g16/scratch/psr_data')
    try:
        os.mkdir(data_path)
    except FileExistsError:
    ...
    plt.title(f'NANOGrav {yr}-year CURN GWB Variance Sensitivity Curve')
    plt.loglog(freqs, h_sc_med, label='Med Stoch', lw=1, c='blue')
    plt.loglog(freqs, h_dsc_med, label='Med Det', lw=1, c='red')
    ...
    plt.axvline(x=irn_harms/Tspan, label=fr'$\frac{{{irn_harms}}}{{\mathrm{{Tspan}}}}$', c='fuchsia', linestyle='--')
    plt.legend(loc="upper left")
    plt.savefig(path+'/sc_h_c_sigma.svg')
    plt.show()
    plt.close()
    ...
    plt.axvline(x=irn_harms/Tspan, label=fr'$\frac{{{irn_harms}}}{{\mathrm{{Tspan}}}}$', c='fuchsia', linestyle='--')
    plt.legend(loc="upper left")
    plt.savefig(path+'/sc_h_c_total.svg')
    plt.show()
    plt.close()
